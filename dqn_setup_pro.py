...
class DQN_FS:
    ...
    def get_reward(self,time_lst):
        reward = -1*(time_lst[2]+time_lst[1]) + 10
    ...
    def cal_loss(self,batch):
        ...
        for data in batch:
            state, action, reward, next_state, next_greedy_action = data
            # print(state, " :: state")
            # print(action, " :: action")
            # print(reward, " :: reward")
            ...
            if len(nsa_input) != 0:
                tgt_q = reward + ((not done) * self.discount_factor * self.target(nsa_input)[next_greedy_action])
                # print(next_greedy_action)
