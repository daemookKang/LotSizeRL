...
class DQN_FS:
    def __init__(self, model=None, learning_rate=10**(-5),decay_facotor=0.99,Min_suard_gradient= 10**(-6),epsilon=0.5,e_decay=10**(-4)
                 ,replay_buffer_size=10**5,batch_size=64,discount_rate=0.9,train_step=20,weight_inv=0.1,env_name = "3"):
        self.model = model
    ...
    def get_sars(self,state_lst,action,next_action,time_lst):
        sars_lst = []
        # state_lst = []
        # action_lst = []
        # reward_lst = []
        ...
        inv_feats = []
        for i in range(-18,-9):
            inv_feats.append(next_state[i]*100)
        ...
        reward = self.get_reward(time_lst,inv_feats)

        sars = this_state, this_action, reward, next_state, next_greedy_action,discount_step
    ...
    def get_reward(self,time_lst,inv_feats):
        # reward = -1*(time_lst[4]+time_lst[5]*self.weight_inv)
        inv_num = 0
        for i in inv_feats:
            inv_num += i*0.05
        reward = -1*(time_lst[4]) + inv_num
        return reward
    ...
    def train(self,state_lst,action,next_action,time_lst):
        ...
        self.eps = max(0.01,(self.eps - 10**(-4))) #linear?? or multiple??
    ...
    def net_train(self):
        ...
        if self.now_step%1000 == 0:
            ...
            torch.save(self.model.state_dict(),
                       'dqntest228job_{}lr_{}df_{}epochs_weight{}_{}.pt'.format(self.learning_rate, self.discount_factor, self.now_step,self.weight_inv,self.env_name))
    ...
    def cal_loss(self,batch):
        ...
        for data in batch:
            state, action, reward, next_state, next_greedy_action,discount_step = data
            # print(state, " :: state")
            # print(action, " :: action")
            # print(reward, " :: reward")
            # print(next_state, " :: next_state")
            # print(data)
            if next_greedy_action == 'no':
                done = True
            else:
                done = False
            ...
            if len(nsa_input) != 0:
                tgt_q = reward + ((self.discount_factor**discount_step) * self.target(nsa_input)[next_greedy_action])
                # print(next_greedy_action)
