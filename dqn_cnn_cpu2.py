...
class DQN_FS:
    def __init__(self, model=None, learning_rate=10 ** (-5), decay_facotor=0.99, Min_suard_gradient=10 ** (-6),
                 epsilon=0.3, e_decay=10 ** (-4)
                 , replay_buffer_size=10 ** 4, batch_size=64, discount_rate=0.9, train_step=20, weight_inv=0.1,
                 env_name="3"):
    ...
    def train(self, state_lst, action, next_action, time_lst):
        ...
        self.eps = max(0.001, (self.eps - 10 ** (-4)))  # linear?? or multiple??
