            ...
            if action_index == self.fab_env.job_index_dic[self.init_setup_status]:
                ...
                for job in self.fab_env.job_dict.values():
                    ...
                    if self.name in job.process_time.keys():
                        demand_now.append(job.demand_time / job.process_time[self.name])
                        if max_d < job.demand_time / job.process_time[self.name]:
                            max_d = job.demand_time / job.process_time[self.name]
                ...
                for job in range(len(self.fab_env.job_dict.values())):
                    demand_now[job] = demand_now[job] / max_d
...
class FABEnvironment:
    def __init__(self, env_type: str = 'dqn', dqn_agent=None,run_time=5000):
        ...
        self.seed_lst = [seed for seed in range(100)]
    ...
    def simulation_run(self,  max_scenario_num=5000):
...
#
dqn = DQN_FS(model,learning_rate=10**(-3),env_name="88")
#
#
env = FABEnvironment(dqn_agent=dqn,run_time=1440)
# # env.set_scenario()
env.simulation_run()
