...
class Net_MLP(nn.Module):
    def __init__(self, num_feats,heads,concat,negative_slope,dropout,add_self_loops,bias):
        ...
        self.theta_v2 = nn.Linear(1, num_feats, bias=False).to(device)


        self.theta_v3 = nn.Linear(num_feats*heads, num_feats, bias=False).to(device)
    ...
    def forward(self, g,v, e,m,batch_size=1, Train = True,only_clast=False):
        ...
        g.mu_total = torch.tensor([[0.]for j in range(len(g.x))])

        for i in range(0,len(g.index_m)):
            if len(g.index_m[i]) > 1:
                g.mu_total += self.embeddingMachine2(g, i+1)
            else:
                continue

        g.mu_total += 0.0000001

        g.mu_total = self.theta_v2(self.embeddingMachine2(g,m) * self.embeddingMachine2(g,m) / g.mu_total)

        g.mu2 = self.theta_v(self.embeddingMachine2(g,m))

        g.mu_total[v] = g.mu2[v]
        ...
        g.notj[v] = 1
        g.mu_total = g.mu_total * g.notj[v]
