...
class Machine:
    ...
    def set_init(self,fab_env,valid):
        ...
        if self.init_setup_status == None:
            ...
            self.fab_env.init_set_lst.append(init_setup.id[1])
                ...
                if self in job.available_M_List:
                    ...
                    if inv <= remaing_t + 3:
                        if inv < minv:
                            ...
                            minv = inv
                            nowd = job.demand_time
                            action = j
                        elif inv == 0:
                            if nowd < job.demand_time:
                                nowd= job.demand_time
                                action = j
    ...
    def simulation_run(self,  max_scenario_num=20):
        ...
        if self.env_type == 'heu':
            ...
            for scenario in range(max_scenario_num):
                ...
                if scenario >= 0:
                    self.valid_lst.append(self.idle_time)
                    self.valid_setup_lst.append(self.setup_chage_num)
                    self.valid_make.append(self.total_make)
                    print(self.idle_time, " result")
                    print(self.valid_lst , " valid")
                    print(self.valid_setup_lst, " setup")
            ...
            print("init_set_lst = ",self.init_set_lst)
            print("mean_make = ", np.mean(np.array(self.valid_make)))
...
lot_size= 100
time_st = time.time()
env = FABEnvironment(env_type='heu',dqn_agent=dqn,run_time=1440,lot_size=lot_size)
