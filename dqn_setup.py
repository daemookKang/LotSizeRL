...
class DQN_FS:
    def __init__(self, model=None, learning_rate=10**(-5),decay_facotor=0.99,Min_suard_gradient= 10**(-6),epsilon=0.5,e_decay=10**(-4)
                 ,replay_buffer_size=10**5,batch_size=64,discount_rate=0.9,train_step=20,env_name = "3"):
        self.model = model
    ...
    def get_reward(self,time_lst):
        reward = -1*(time_lst[4]+time_lst[3])
    ...
    def net_train(self):
        ...
        if self.now_step%1000 == 0:
            ...
            torch.save(self.model.state_dict(),
                       'dqntest228job_{}lr_{}df_{}epochs{}.pt'.format(self.learning_rate, self.discount_factor, self.now_step,self.env_name))
