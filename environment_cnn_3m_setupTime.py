...
from cnn_low_deuling_1conv import NN_FABSched
    ...
    def simulation_run(self, max_scenario_num=3002):
        ...
        if self.env_type == 'dqn':
            ...
            for scenario in range(max_scenario_num):
                ...
                if scenario % 51 == 0 and scenario > 0:
                    if min_val > self.total_idle:
                        min_val = self.total_idle
                        self.dqn.save_model()
...
lr_lst = [10**(-3)]
print(lr_lst)
df_lst = [0.99, 0.98, 0.97, 0.992,0.965]
valid_lst = {}
setup_lst = {}
    ...
    for lr in lr_lst:
        for df in df_lst:
            ...
            dqn = DQN_FS(model, learning_rate=lr, discount_rate=df,
                         env_name="_3m9j_noset_new_cpu_setup_netdemand_deuling_cnn_960day_{}".format(lot_size))
            #
            #
