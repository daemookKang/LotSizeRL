...
from cnn_low import NN_FABSched
...
def scenario_generator(env, machine_num: int = 35, num_job=4, seed=0, valid=False) -> (
    ...
    for job_type in range(num_job):
        ...
        if len(group) < 13 and len(group_job) < 2:
            group.append(job_id)
            if len(group) == 12:
                group_job.append(copy.deepcopy(group))
                group = []

        elif len(group_job) >= 2:
            group.append(job_id)
            if len(group) == 13:
                group_job.append(copy.deepcopy(group))
                group = []
            ...
            if job1.id == job2.id:
                ...
                if job1.id in group_job[0]:
                    if job2.id in group_job[0]:
                        set_up_time2 = 8
                    elif job2.id in group_job[1] or job2.id in group_job[3]:
                        set_up_time2 = 12
                ...
                elif job1.id in group_job[3]:
                    if job2.id in group_job[3]:
                        set_up_time2 = 8
                    elif job2.id in group_job[0] or job2.id in group_job[2]:
                        set_up_time2 = 12
    ...
    for lr in lr_lst:
        for df in df_lst:
            ...
            dqn = DQN_FS(model, learning_rate=lr, discount_rate=df,
                         env_name="_11m50j_noset_cpu_setup_netdemand_cnn_960day_{}".format(lot_size))
            #
            #
