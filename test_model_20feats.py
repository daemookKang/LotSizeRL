            ...
            if chk * 1440 <= self.env.now:
                for job in self.fab_env.job_dict.keys():
                    ...
                    for job in self.fab_env.job_dict.values():
                        ...
                        # self.fab_env.total_demand.update({job.id: int(1440 / job.demand_time) })
                        # self.fab_env.max_demand.update({job.id: int(1440 / job.demand_time) })
...
class FABEnvironment:
    ...
    def set_scenario(self,seed,valid,init_inv=None):
        ...
        for job in self.job_dict.values():
            ...
            self.max_demand.update({job.id: int(2880 / job.demand_time)})
...
# model.load_state_dict(torch.load("no_setup_numreward\dqntest228job_0.01lr_0.9df_5000epochs_weight0.1__new_demand_numreward_1m_30.pt"))
model.load_state_dict(torch.load("setup_numreward\dqntest228job_0.001lr_0.99df_5000epochs_new_demand_numreward_setup_1m_20.pt"))
# model.load_state_dict(torch.load("oneday_setup\dqntest228job_0.001lr_0.9df_5000epochs_new_onedaydemand_numreward_setup_1m_20.pt"))
# model.load_state_dict(torch.load("oneday_setup\dqntest228job_0.01lr_0.9df_5000epochs_new_onedaydemand_demand_numreward_setup_1m_30.pt"))

model.load_state_dict(torch.load("no_setup_oneday\dqntest228job_0.001lr_0.9df_5000epochs_weight0.1__new_onedaydemand_numreward_nosetup_1m_20.pt"))
# model.load_state_dict(torch.load("no_setup_oneday\dqntest228job_0.01lr_0.9df_5000epochs_weight0.1__new_onedaydemand_demand_numreward_nosetup_1m_30.pt"))
# model.load_state_dict(torch.load("dqntest228job_0.001lr_0.9df_5000epochs_weight0.1__new_onedaydemand_demand_numreward_nosetup_1m_50.pt"))

# model.load_state_dict(torch.load("dqntest228job_0.01lr_0.999df_5000epochs_weight0.1__new_onedaydemand_demand_numreward_nosetup_1m_20.pt"))

# model.load_state_dict(torch.load("dqntest228job_0.001lr_0.999df_5000epochs_new_onedaydemand_demand_numreward_setup_1m_40.pt"))
dqn = DQN_FS(model,learning_rate=10**(-3),env_name="test")
lot_size = 20
#
#
env = FABEnvironment(env_type='dqn',dqn_agent=dqn,run_time=1440,lot_size=lot_size)
# # env.set_scenario()
env.simulation_run()
