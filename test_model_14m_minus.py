...
class Machine:
    ...
    def set_init(self, fab_env, valid, SS):
        self.fab_env = fab_env
        self.setup_status = []
        can_setup = []
        state = []
        ...
        if self.fab_env.env_type == 'dqn':
            ...
            for job in self.fab_env.job_dict.values():
                ...
                if self.name in job.process_time.keys():
                    ...
                    if max_d < job.demand_time:
                        max_d = job.demand_time
                    if max_i < self.fab_env.inventory_dic[job.id] :
                        max_i = self.fab_env.inventory_dic[job.id]
                else:
                    demand_now.append(0)
                    process_t.append(0)
                    inventory_now.append(-1)
...
model.load_state_dict(torch.load("dqntest228job_0.01lr_0.99df_1000epochs_weight0.1__14m45j_noset22_new_backorder_960day_20.pt"))

# model.load_state_dict(torch.load("dqntest228job_0.01lr_0.99df_1000epochs_weight0.1__3m9j_noset_new_demand_960day_20.pt"))
