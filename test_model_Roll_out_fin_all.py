...
from Graph_make_full_flexible import make_dataset2
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('gatfinals22_201127_3.pt')) #gat2
    model.eval()
    scen_num = 100
    for sena in range(scen_num):
        ...
        for i in range(len(roll_out)):

            new_v, fin_node, fin_edge, fin_job,fin_Machine, fin_m_ind, fin_work_ind, job_c_dic, c_list, C2_list,now_job_list,done_node_list, done_edge_list, now_oper_list, can_working_oper = roll_out[i]
    ...
    plt.plot(list(range(scen_num)), ratio2, label='eailies c')
    plt.plot(list(range(scen_num)), ratio3, label='roll_out 2')
