...
from cnn_low_deuling2 import NN_FABSched
...
class Machine:
    ...
    def last_action_F(self, total_back_order, time_lst, action_index):
        ...
        for i in range(self.fab_env.num_job):
            ...
            setup_me.append(self.fab_env.set_up_time_dic[key] / 10)
    ...
    def process(self):
        while True:
            ...
            if action_index == self.fab_env.job_index_dic[self.init_setup_status]:
                ...
                if self.valid == False:
                    self.fab_env.dqn.train(self.state_lst, action_index, action_space, time_lst)
...
def scenario_generator(env, machine_num: int = 35, num_job=4, seed=0, valid=False) -> (
    ...
    for job_type in range(num_job):
        available_M_List = []
        if job_type < 2:
            available_M_List = group_m[0]
        elif job_type < 4:
            available_M_List.extend(group_m[0])
            available_M_List.extend(group_m[1])
        elif job_type < 6:
            available_M_List.extend(group_m[1])
            available_M_List.extend(group_m[2])
        else:
            available_M_List = group_m[2]
        # print(available_M_List)
        # continue
...
class FABEnvironment:
    ...
    def set_scenario(self, seed, valid):
        ...
        demand_lst = [5, 10]
        self.machine_dict, self.job_dict, self.set_up_time_dic, self.back_order_dic = scenario_generator(self.sim_env,
                                                                                                         self.machine_num,
                                                                                                         self.num_job,
                                                                                                         seed=1,
                                                                                                         valid=valid)
    ...
    def simulation_run(self, max_scenario_num=3002):
        self.min_valid = 9999999999999
            ...
            for scenario in range(max_scenario_num):
                ...
                if scenario % 51 == 0 and scenario > 0:
                    ...
                    for v_s in range(10):
                        ...
                        valid = True
                        self.set_scenario(50000+v_s, True)
                        ...
                        self.sim_env = simpy.Environment()
                        valid_idle.append(self.total_idle)
                        valid_change.append(self.setup_chage_num)
                        print(valid_idle)
                    ...
                    self.sim_env = simpy.Environment()
                self.dqn.now_step += 1
                print(self.total_idle)
                print(self.setup_chage_num)
                self.dqn.decaying_eps()
                if scenario % 10 == 0 and scenario > 0:
                    self.dqn.update_dqn()

                if scenario % 51 == 0 and scenario > 0:
                    mean_valid = np.mean(np.array(valid_idle))
                    if mean_valid < self.min_valid:
                        self.min_valid = mean_valid
                        self.dqn.save_model()

                    self.valid_lst.append(mean_valid)
                    self.valid_setup_lst.append(np.mean(np.array(valid_change)))
...
for lot_size in lot_size_lst:
    for lr in lr_lst:
        for df in df_lst:
            ...
            #
            dqn = DQN_FS(model, learning_rate=lr, discount_rate=df,
                         env_name="_3m9j_noset_new_set_cnn_960day_{}".format(lot_size))
            #
            #
